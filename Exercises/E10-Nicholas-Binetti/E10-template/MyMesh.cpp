...
void MyMesh::computeAABB(const std::vector<vec3> temp_verts)
{
	if (temp_verts.empty()) return;

	// Initialize minVert and maxVert with the first vertex
	minVert = maxVert = temp_verts[0];

	for (const vec3& v : temp_verts) {
...
void MyMesh::drawAABB()
{
	...
	vec3 v0 = vec3(minVert.x, minVert.y, minVert.z);
	vec3 v1 = vec3(maxVert.x, minVert.y, minVert.z);
	vec3 v2 = vec3(maxVert.x, maxVert.y, minVert.z);
	vec3 v3 = vec3(minVert.x, maxVert.y, minVert.z);
	vec3 v4 = vec3(minVert.x, minVert.y, maxVert.z);
	vec3 v5 = vec3(maxVert.x, minVert.y, maxVert.z);
	vec3 v6 = vec3(maxVert.x, maxVert.y, maxVert.z);
	vec3 v7 = vec3(minVert.x, maxVert.y, maxVert.z);

	// Draw edges
	// Bottom face
	glVertex3fv(value_ptr(v0)); glVertex3fv(value_ptr(v1));
	glVertex3fv(value_ptr(v1)); glVertex3fv(value_ptr(v2));
	glVertex3fv(value_ptr(v2)); glVertex3fv(value_ptr(v3));
	glVertex3fv(value_ptr(v3)); glVertex3fv(value_ptr(v0));

	// Top face
	glVertex3fv(value_ptr(v4)); glVertex3fv(value_ptr(v5));
	glVertex3fv(value_ptr(v5)); glVertex3fv(value_ptr(v6));
	glVertex3fv(value_ptr(v6)); glVertex3fv(value_ptr(v7));
	glVertex3fv(value_ptr(v7)); glVertex3fv(value_ptr(v4));

	// Vertical edges
	glVertex3fv(value_ptr(v0)); glVertex3fv(value_ptr(v4));
	glVertex3fv(value_ptr(v1)); glVertex3fv(value_ptr(v5));
	glVertex3fv(value_ptr(v2)); glVertex3fv(value_ptr(v6));
	glVertex3fv(value_ptr(v3)); glVertex3fv(value_ptr(v7));
