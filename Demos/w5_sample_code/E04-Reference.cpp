...
void keyboard(unsigned char key, int x, int y)
{
	...
	const float barWidthHalf = barSize / 2.0f;
	...
	case 'a':
		...
		barPos[0] = std::max(barPos[0] - step, barWidthHalf);
		// write the code above
		break;
	case 'd':
		// write the code below
		barPos[0] = std::min(barPos[0] + step, canvasSize[0] - barWidthHalf);
		// write the code above
		break;
	}
}
...
void update()
{
	...
	{
		// write the code below
		if (ballPos[1] - radius > barPos[1] && ballPos[1] + velocity_y * deltaTime - radius < barPos[1] && ballPos[0] + radius > barPos[0] - barSize / 2.0f && ballPos[0] - radius < barPos[0] + barSize / 2.0f)
		{
			velocity_y = abs(velocity_y);
		}
		// write the code above

		if (ballPos[0] - radius < 0.0f)
			velocity_x = abs(velocity_x);
		else if (ballPos[0] + radius > canvasSize[0])
			velocity_x = -abs(velocity_x);
		...
		else if (ballPos[1] + radius > canvasSize[1])
			velocity_y = -abs(velocity_y);
